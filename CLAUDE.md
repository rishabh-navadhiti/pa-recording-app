...
|---|---|---|
| `<repo>/.env` | App writes; user-editable | `ELEVENLABS_API_KEY`, `NOTES_DIR_PATH`, plus the LLM API keys written via the secrets store (`config/secrets.js`): `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `OPENAI_API_KEY` |
| `<NOTES_DIR>/settings.json` | App writes; user-editable | `autoRecord`, `manualDeviceSelection`, `selectedDeviceIndex`, `soapModel` (a note-gen **option id** from `NOTE_GEN_OPTIONS` in `src/llm/modelOptions.js` — each id maps to a `{provider, model}`: `sonnet-4-6-api`/`sonnet-4-6-agentic` → Anthropic, `gemini-3.5-flash` → Gemini, `gpt-5.6-luna` → OpenAI; the SOAP step routes to `ctx.api`/`ctx.llm`/`ctx.gemini`/`ctx.openai` accordingly, while the post-SOAP engines always resolve to an Anthropic model via `resolveCliModel`), `templateModel`, `templateEffort`, `enableIcd` (gates the per-case ICD coding step), `enableCdi`, `cdiMode` (the last two gate the per-case CDI review step), `enableEmScore` (gates the per-case E/M MDM scoring engine), `enablePatientSummary` (gates the per-case patient-summary engine). **Invariant: `enableCdi` on ⟹ `enableIcd` on** — CDI needs ICD codes in the note, so enabling CDI forces ICD on (enforced in `config/settings.js` — the settings store's normalizer — and the `save-settings` handler in `src/ipc/config.js`). The two new toggles are independent (no coupling). — **`doctors[]` moved to `app.db` after first launch** |
...
- Skills: `notes-claude/skills/{generate-note,generate-note-api,create-doctor-profile,update-doctor-profile,edit-note,edit-note-api,add-icd-codes,cdi-review,cdi-costigan,cdi-costigan-api,em-score,em-score-api,patient-summary,patient-summary-api}/SKILL.md` (`generate-note-api` has an optional PRE-CHART CONTEXT rule that engages only when the app injects a pre-chart block; there is no separate prechart skill. `em-score`/`patient-summary` are the agentic CLI skills, left on disk but dormant; the engines run the `*-api` skills as single Anthropic API calls — see DECISIONS 2026-06-29)
...
- Default models (overridable via settings.json): SOAP = `claude-sonnet-4-6` (option `sonnet-4-6-api`; `DEFAULT_OPTION_ID` unchanged), template = `claude-opus-4-8` (effort=max). Selectable note-gen options: Anthropic (API + agentic), `gemini-3.5-flash` (Gemini), `gpt-5.6-luna` (OpenAI). Non-Anthropic SOAP providers still run the post-SOAP engine chain on Anthropic.
