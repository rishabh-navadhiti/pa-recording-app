...
- Skills: `notes-claude/skills/{generate-note,generate-note-api,create-doctor-profile,update-doctor-profile,edit-note,edit-note-api,add-icd-codes,cdi-review,cdi-costigan,cdi-costigan-api,em-score,patient-summary}/SKILL.md` (`generate-note-api` has an optional PRE-CHART CONTEXT rule that engages only when the app injects a pre-chart block; there is no separate prechart skill)
...
- E/M MDM scorer (`em-score`) + Patient summary (`patient-summary`) run after CDI, each gated by `enableEmScore` / `enablePatientSummary`. **JSON only** (`<case>_em.json` / `<case>_patient_summary.json`, no MD/docx — presentation renders from JSON in a later step). Each persists one row to the generic `engine_outputs` table (`db/engine_outputs.js`), NOT per-`cases` columns. em-score is connector-free (AMA/CPT rules); patient-summary is plain-language, connector-free.
