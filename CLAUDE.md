...
7. On SOAP close: `parseSkillManifest()` reads the manifest from the skill's final assistant text. Then, **per case folder** (single-patient: the parent case folder; multi-patient: each child case folder, sequentially), the app runs the per-case post-processing chain: **ICD coding → docx conversion**.
   - **Single-patient** (`multi_patient: false`): `spawnIcdCoding` appends an `## ICD-10-CM Codes` table to the declared `.md` (best-effort — failure logs + emits `service-warning` IPC but the pipeline still continues). Then `spawnDocxConversion` runs against the same `.md` — generates the `.docx` (now with ICD codes baked in), hides the `.md` on Windows, updates the existing `cases` row to `completed`. `transcript.docx` is generated in parallel after transcription as before.
   - **Multi-patient** (`multi_patient: true`): for each `ok`/`partial` entry in `cases[]`, the app creates a child folder next to the recording folder using the single-patient convention (`<slug>_<YYYY-MM-DD>/`), copies the parent's MP3, `transcript.md`, and `transcript.docx` in with single-patient naming, copies the SOAP `.md` in renamed to `<folder>_soap_note.md`, inserts a child `cases` row, **awaits `spawnIcdCoding` on the child's `.md`**, then spawns docx on the child. ICD runs are sequential across children (one at a time) so the MCP connector and Anthropic quota aren't hit in parallel and per-case log blocks don't interleave. The recording (audit) folder retains the original MP3, transcript, and all the `.md` SOAP files the skill wrote — **never ICD-coded, never docx-converted**; on Windows those `.md` files are hidden. The parent row is marked `completed` with `soap_note_path=NULL`. Non-DB manifest fields (`visit_type`, `chief_complaint`, `placeholders`, `warnings`, `summary`) are logged to `app.log` only.
