...
- Skills: `notes-claude/skills/{generate-note,generate-note-api,create-doctor-profile,update-doctor-profile,edit-note,edit-note-api,add-icd-codes,cdi-review,cdi-costigan,em-score,patient-summary}/SKILL.md` (`generate-note-api` has an optional PRE-CHART CONTEXT rule that engages only when the app injects a pre-chart block; there is no separate prechart skill)
