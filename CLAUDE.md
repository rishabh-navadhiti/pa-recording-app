...
notes-claude/                   Bundled Claude Code workspace — copied at runtime to <NOTES_DIR>/.claude
  ...
  skills/add-icd-codes/           RETIRED / reference-only (invoked by nothing) — the old agentic ICD-10 skill (claude.ai ICD-10 MCP connector). ICD now runs as one Anthropic API call (add-icd-codes-api) + local-codeset validation; see src/icd/ + data/icd/ below
...
src/                          Modular app code (Phases 0-5). main.js is now a thin bootstrap + shims + a deps-assembling registerIpcHandlers.
  ...
  icd/                          local-codeset ICD path: lookup.js (better-sqlite3 read-only over data/icd/*.db, auto-selects newest FY) + coder.js (cross-check the API candidates + render the ## ICD-10-CM Codes table). Consumed by src/engines/icd.js runLlm.
...
data/icd/                     Bundled FY2026 ICD-10-CM codeset: icd10cm_fy2026.db (~21 MB committed binary, no long_desc), build_icd_db.py (regen from the CMS order file), README.md (refresh policy). Read read-only by src/icd/lookup.js.
...
7. **Costigan checklist (opt-in, `enableCostiganCdi`).** After the edit-note step completes (both API and CLI paths), if `enableCostiganCdi` is on and the case's doctor is Costigan, the pre-chart `onSuccess` fires a single Anthropic API call (`cdi-costigan-api` skill — system prompt = SKILL.md body + the 5 procedure packs concatenated; user message = final SOAP note + pasted Box-B chart; model `claude-sonnet-4-6`, no tools, no connector). It writes `<stem>_costigan.json` + `<stem>_costigan.md` + a **visible** `<stem>_costigan_report.html` + `<stem>_costigan_report.pdf` (the Clinical-Cockpit-styled deliverable the scribe opens — HTML rendered from the JSON by `renderCostiganHtml()` in [src/render/costiganHtml.js](src/render/costiganHtml.js), sibling of the `.md` renderer — it maps the procedure-checklist shape onto the **shared Clinical-Cockpit design** (inlines `src/render/cockpit.css`, the stylesheet extracted verbatim from the combined CDI/E·M/patient-summary report, so the Costigan report matches that look: navy header, flag-badges, flag-cards, evidence boxes, code-validation table); the PDF is the same HTML printed via `htmlToPdf()` in [src/render/htmlToPdf.js](src/render/htmlToPdf.js) — an offscreen Electron `printToPDF`, best-effort, falls back to the HTML if it fails) (+ `<stem>_chart_input.md` if chart text was supplied), and records one `processing_events` row with `job_kind='costigan'`. The `.json`/`.md`/`_chart_input.md` are hidden on Windows; **the `_costigan_report.{html,pdf}` are left visible** (not added to `writtenArtifacts`). **No DB schema change.** Because the checklist runs *detached* from the pre-chart job (fired, not awaited), on completion it pushes a one-way **`costigan-report-ready`** event (`{patient, overallStatus, reportPath, caseDir}`) to the renderer, which shows a global **"Open report"** banner ([renderer/views/costiganBanner.js](renderer/views/costiganBanner.js)); opening reuses the existing `open-soap-note` IPC (the report lives inside `casesDir`). Box A (edit-note instructions) is unchanged — only Box B's chart text feeds the checklist; an extra "Epic chart" text area appears in the Pre-chart view when `enableCostiganCdi` is on for a Costigan doctor.
...
Events (`on*`):
`onStateChange`, `onShowPatientForm`, `onSetupWarning`, `onAutoStartRecording`, `onPickDoctor`, `onServiceWarning`, `onTemplateJobStatus`, `onRecordingStatusUpdate` (driving the floating status window), `onCostiganReportReady` (Costigan checklist HTML/PDF report finished → "Open report" banner).
...
- Skills: `notes-claude/skills/{generate-note,generate-note-api,create-doctor-profile,update-doctor-profile,edit-note,edit-note-api,add-icd-codes-api,add-icd-codes,cdi-review,cdi-costigan,cdi-costigan-api,em-score,em-score-api,patient-summary,patient-summary-api}/SKILL.md` (`generate-note-api` has an optional PRE-CHART CONTEXT rule that engages only when the app injects a pre-chart block; there is no separate prechart skill. `em-score`/`patient-summary` are the agentic CLI skills, left on disk but dormant; the engines run the `*-api` skills as single Anthropic API calls — see DECISIONS 2026-06-29. `add-icd-codes-api` is the ICD engine's Phase-A API skill (proposes candidates, then Node validates vs the local codeset); `add-icd-codes` is the retired/dormant agentic ICD skill, kept on disk but invoked by nothing — see DECISIONS 2026-07-01)
