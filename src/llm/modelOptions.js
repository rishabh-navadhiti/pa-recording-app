...
// `sonnet-4-6-api`) or a non-Anthropic model (e.g. the `openai` gpt-5.6-luna)
// straight to the CLI makes it 404 ("model may not exist"). This resolves the id
// to its underlying Anthropic model for the `api`/`cli` providers, and falls back
// to the default Anthropic model when the SOAP selection is a provider the CLI
// can't run (openai) or an unknown/legacy value — the engines always run on
// Anthropic regardless of the note-gen selection.
