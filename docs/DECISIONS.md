...
**Non-goals / kept:** The `generateSoapViaApi` `provider`/`providerName` parameters stay (always `ctx.api` / `'Anthropic'` now) to keep the diff contained — they are inert, not dead. User `.env` files may still hold a `GEMINI_API_KEY`; it is simply ignored. **Tests:** dropped the Gemini-fallback case in `model-options.test.js` (the unknown/legacy fallback test still covers `resolveCliModel`'s default path), and the registrar handler count drops `49 → 47` (two `save/get-gemini-key` handlers removed). The `shared-drift` preload↔`CHANNELS` guard stays green because both sides lost the two channels.
