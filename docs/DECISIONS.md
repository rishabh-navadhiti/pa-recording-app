...
## 2026-07-01 — ICD coding moves to a single API call + a bundled local codeset (retires `claude -p`) (rs)

**Context.** ICD coding was the last agentic `claude -p` engine and the only one still using the claude.ai ICD-10 MCP connector (`add-icd-codes` skill + `.mcp.json`). A hands-on test of the managed-MCP Messages-API path on a real note took ~21 min and ~83k input tokens for ONE note with repeated 300s connector timeouts — too slow/flaky for the pipeline. The connector's value (ground-truth existence + billable status + the no-upgrade-unless-a-more-specific-billable-child-exists discipline) is fully reproducible offline from the CMS ICD-10-CM order file.

**Decision.** Replace the agentic ICD step with a two-phase, mostly-deterministic path, and remove the `claude -p` ICD path from code. ICD stays a `runEngine` engine but exposes `runLlm` (like em-score/patient-summary), so every call site (chain.js, prechart.js, prechartApi.js) is unchanged. Phase A: `add-icd-codes-api` skill proposes billable candidate codes from the note in one Anthropic API call (pinned to `ctx.api`). Phase B: `src/icd/coder.js` cross-checks each candidate against a bundled FY2026 SQLite codeset (`src/icd/lookup.js` over `data/icd/icd10cm_fy2026.db`, better-sqlite3, read-only) — **dial = cross-check**: accept only if it exists + is billable + description matches; else re-resolve via a codeset search on the model's search terms, else FLAG for manual coding (never emit a wrong code). Phase C: append (or replace, on a pre-chart re-run) the same `## ICD-10-CM Codes` table in the SOAP `.md` + write `<stem>_icd.json`.

**Removed (agentic ICD path unsupported):** the `add-icd-codes` prompt builder in `src/llm/skill-io/prompts.js` (`buildPrompt('add-icd-codes')` now throws — guarded in prompts.test.js). `notes-claude/skills/add-icd-codes/` is kept on disk for reference only, invoked by nothing. `.mcp.json` / the ICD-10 connector stays (cdi-review still uses it; the runEngine MCP-error service-warning was repointed `icd` -> `cdi`).

**DB distribution.** The app runs from the git checkout and auto-updates via `git pull`, so the codeset ships as a committed binary (`data/icd/icd10cm_fy2026.db`, ~21 MB; `long_desc` dropped to keep it small) — no packaging/download step. `lookup.js` auto-selects the newest `icd10cm_fy<YEAR>.db`; a fiscal-year refresh is just dropping a new file (regen: `data/icd/build_icd_db.py`; see `data/icd/README.md`). Parity-test against the live connector before trusting a build.

**Provider-agnostic upside.** Phase B is pure JS, so Phase A can move to Gemini/DeepSeek later without losing the validator — unlike the managed-MCP connector, which is Anthropic-only.
