...
## 2026-07-21 — gpt-5.6-luna added as an OpenAI SOAP note-gen provider (sr)

**Context:** A 27-run bake-off validated OpenAI's gpt-5.6-luna for SOAP note generation. Added it as an opt-in, selectable note-gen option alongside the existing Anthropic (API + agentic) and Gemini providers. Default (`sonnet-4-6-api`) is unchanged; rollout/default-flip is a later decision.

**Decisions:**

1. **New `src/llm/openaiApiProvider.js`, cloned from `geminiApiProvider.js`** (same `runSingleCall` / always-resolves contract). Two load-bearing differences from the Gemini OpenAI-compat clone, both validated by the bake-off against the real OpenAI endpoint: the body uses `max_completion_tokens` (the gpt-5 family **rejects** `max_tokens`), and `reasoning_effort` is pinned to `'minimal'` (higher effort measurably degraded notes — the plan leaked into the HPI). Effort is never exposed as a setting. `'none'` is deliberately avoided (documented to misbehave alongside `max_completion_tokens` on some GPT-5 versions). **Doc caveat:** some GPT-5.6 docs list effort values without `minimal`, but the bake-off confirmed `minimal` is accepted and behaves as minimal for gpt-5.6-luna — if the API ever rejects it, revisit the provider rather than silently falling back.

2. **Usage mapped in-provider, not a pricing fork.** The provider emits `rawUsage` in the Anthropic-shaped keys `src/llm/pricing.js` already consumes (`input_tokens` = `prompt_tokens` incl. cached; `output_tokens` = `completion_tokens` incl. reasoning — NOT double-counted; `cache_read_input_tokens` = `prompt_tokens_details.cached_tokens`). So `calcCost`/`normalizeApiUsage` need no per-provider branch — only a `PRICE_TABLE['gpt-5.6-luna']` row (`{in:1, out:6, cacheRead:0.10, cacheWrite:0}`), keeping `processing_events.cost_usd` populated. (Note: Gemini has no price row, so its cost is null today; luna is the first non-Anthropic provider with real pricing.)

3. **Registered as option `gpt-5.6-luna`** (`{provider:'openai', model:'gpt-5.6-luna'}`) in `NOTE_GEN_OPTIONS`; `DEFAULT_OPTION_ID` unchanged. `resolveCliModel` already falls back to the default Anthropic model for non-`api`/`cli` providers, so the post-SOAP engine chain (ICD/CDI/em/patient-summary) + the CLI prechart stay on `claude-sonnet-4-6` regardless of the SOAP selection — no change needed.

4. **Wiring mirrors Gemini exactly:** `ctx.openai` in `appContext`, `secrets.get/setOpenAiKey()` (`.env` `OPENAI_API_KEY`), `validateOpenAiKey` (GET `/v1/models`, Bearer), `get/save-openai-key` IPC channels + config handlers + preload, and an OpenAI key field + a "GPT-5.6 Luna" option in the (hardcoded) renderer Settings dropdown. The SOAP provider ternary (`main.js` `spawnSoapGeneration`) and the auth/rate-limit key-name ternaries (main.js ×2, plus `src/jobs/prechartApi.js` — the pre-chart edit-note API path, which the original handoff missed) gained an `'openai'`/`'OpenAI'` branch. No DB migration (`model_used` is free text).

5. **Tests:** extended `model-options.test.js` (luna → openai; `resolveCliModel` fallback) and added `pricing.test.js` (luna cost split + non-null `cost_usd`). A dedicated openai-provider unit test was intentionally dropped to match the existing pattern — the sibling HTTP providers (Gemini, Anthropic-API) have no provider unit test either. Live end-to-end (real key + audio) is left as a manual step.

### 2026-07-22 addendum — `reasoning_effort` corrected to `'low'` (handoff's `'minimal'` is rejected) (sr)

First live luna run failed with HTTP 400: `reasoning_effort` "does not support 'minimal' with this model. Supported values are: 'none', 'low', 'medium', 'high', 'xhigh'." The handoff's "pin to `minimal`" was empirically wrong for the model actually behind `gpt-5.6-luna` (a discrepancy the pre-implementation web check had already flagged as possible). The bake-off's real signal was that *higher* effort degrades notes (plan leaks into HPI), so the intent is the **lowest** reasoning available. Changed the pinned value to `'low'` — the closest supported value to the intended minimal. `'none'` was rejected as the choice because it is documented to be silently ignored alongside `max_completion_tokens` on some GPT-5 versions, which would raise effort back up. Provider comment updated to record the live-API constraint; this may warrant a quick note-quality re-check since the bake-off only compared `minimal` vs `medium`, not `low`.

### 2026-07-22 — Removed the Gemini note-generation option entirely (sr)

**Context:** The note-gen model dropdown offered **Gemini 3.5 Flash** (labelled "recommended") alongside the Anthropic Sonnet options, backed by a full parallel provider stack (`src/llm/geminiApiProvider.js`, a `ctx.gemini` provider, `get/save-gemini-key` IPC + preload, `getGeminiKey/setGeminiKey/getGeminiBaseUrl` in secrets, `validateGeminiKey`, and a Gemini API Key settings field). Gemini was never actually the default (`DEFAULT_SETTINGS.soapModel = 'sonnet-4-6-api'`), and the requester asked to drop it completely.

**Decision:** Full removal, not just hiding the dropdown entry. Rationale: leaving the Gemini API Key field visible with no model to consume it, or leaving a dead provider branch, is worse than the extra diff. Removed the `gemini-3.5-flash` registry entry (`modelOptions.js`) + `VALID_SOAP_OPTIONS` (`config/settings.js`), the dropdown `<option>` and the Gemini API Key block (`renderer/index.html`) with its `settingsView.js` wiring, and the entire backend stack (deleted `geminiApiProvider.js`; removed `ctx.gemini`, the `opt.provider === 'gemini'` dispatch in `main.js`/`prechartApi.js`, the IPC channels + handlers, the secrets accessors, the preload methods, and `validateGeminiKey`). `sonnet-4-6-api` is now labelled recommended (it is the default). The `settingsView` fallback and the `providerName === 'Gemini'` error-messaging branches collapse to Anthropic.

**Non-goals / kept:** The `generateSoapViaApi` `provider`/`providerName` parameters stay to keep the diff contained — they are inert, not dead. User `.env` files may still hold a `GEMINI_API_KEY`; it is simply ignored. **Tests:** dropped the Gemini-fallback case in `model-options.test.js` (the unknown/legacy fallback test still covers `resolveCliModel`'s default path), and the registrar handler count drops by two (`save/get-gemini-key` handlers removed). The `shared-drift` preload↔`CHANNELS` guard stays green because both sides lost the two channels.

**Merge note (2026-07-22, sr):** this removal was developed in parallel with, and merged alongside, the gpt-5.6-luna work above. In the merged tree the note-gen options are **OpenAI gpt-5.6-luna + Anthropic Sonnet 4.6 (API/agentic) + Sonnet 5 (API)** — Gemini gone, OpenAI kept. So `generateSoapViaApi`'s `provider`/`providerName` are NOT "always Anthropic": `spawnSoapGeneration`/`prechartApi` still route the `openai` provider to `ctx.openai`/`'OpenAI'`, and the `keyEnvName`/`providerName` error branches keep their `'OpenAI'` arm (only the `'Gemini'` arm was dropped). The registrar handler count nets `+2` (openai) `−2` (gemini) versus the pre-both-merges baseline.
