...
- **`src/skills/contracts/manifest.js`**: move `parseSkillManifest.js` here, add `validateManifest`, expand its test suite.
...
**What moves:** `llm/provider.js` + `claudeCliProvider.js` (arg-array spawn, **no `shell:true`** — this is where the injection bug dies) + `childRunner.js` + `usage.js`; `skills/contracts/prompts.js` + `markers.js` (single rate-limit/MCP source); `engines/engineRunner.js` + `registry.js` + `soap.js`/`icd.js`/`cdi.js` descriptors; `pipeline/chain.js` (single + multi, with `completesCase`). Migrate the skills to the JSON-manifest envelope in lockstep (retire `ICD_OK:`/free-prose/`Updated:`), and move the embedded CDI renderer to `python/render_cdi.py`. Clean repo-internal leakage from `notes-claude/`.
