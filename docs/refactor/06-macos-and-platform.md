...
1. **Packaging:** electron-builder **DMG + ZIP** target; **`@electron/notarize`** in CI; Apple Developer Program **$99/yr**; code-sign + notarize + staple (Gatekeeper blocks otherwise on every Mac since Catalina). The bundled-Python (python-build-standalone has mac builds) and provider-seam paths are already cross-platform, so this is mostly signing + CI config.
