...
| **A5** | **`claude login` stays a separate manual step; subscription auth stays.** | The refactor keeps the `claude` CLI + subscription + manual `claude login` as-is (no auth-model change). The only add is an in-app **auth health-check** (surface "not logged in / expired" instead of silent SOAP failure) — recommended, not required. The SDK/API/vendor migration is explicitly deferred (B1). | rish, 2026-06-03 |
| **A6** | **macOS code-signing: skip for production.** | Production scribes are Windows; Mac = dev machines (run from source, no signing) + a few internal demo machines (right-click→Open bypasses Gatekeeper, free). Do NOT pay Apple's $99/yr unless distributing a Mac build to external non-technical Mac *users* — currently nobody. | rish, 2026-06-03 |
| **A7** | **Port transcribe + extract from Python to Node; keep record.py (and docx until golden-tested).** | `record.py` stays Python (no Node WASAPI/BlackHole equivalent). `transcribe.py` + `extract_attachments.py` port to Node (clear wins — shrink the bundled Python, join the testable core). `md_to_docx.py` stays Python until golden-file tests exist. | rish, 2026-06-03 |

**Implications threaded through the docs:** A1 → [03](03-migration-plan.md) is phased-but-chunky with hard safety gates. A2 → [02 §tooling](02-target-architecture.md). A3 → [02 §LLM seam](02-target-architecture.md) + [04](04-distribution-and-updates.md) (note: the refactor keeps `claude -p`; SDK/vendor is a *future, optional* second provider, not part of this work). A4 → [04](04-distribution-and-updates.md) + [06](06-macos-and-platform.md). A5/A6/A7 → [04](04-distribution-and-updates.md).
...
**B8 — Port the non-audio Python workers to Node?** (rish: the others only stayed in Python because `record.py` needed it — porting where it helps is welcome.)
**Recommendation (refined):** **`record.py` stays Python** (no Node equivalent for WASAPI loopback / BlackHole). **Port `transcribe.py` (HTTPS POST + formatting — clear win, drops `elevenlabs`/`requests`) and `extract_attachments.py` (`mammoth`/`pdf-parse` — easy win)** in Phase 5. **Keep `md_to_docx.py` in Python until golden-file tests exist**, then port behind those tests (it's load-bearing note formatting; a port could subtly change output). Net: bundled Python shrinks to ~`record.py` (+ maybe docx), and two more modules join the testable Node core. Don't port docx without the golden files.
...
**B15 — Frontend framework (for the future unified/prescribed workflow screen).** *(Gates: nothing in this refactor; decide when the unified screen is scoped.)*
The current ~13 screens are simple and the refactor uses a hand-rolled `{mount,update,unmount}` vanilla pattern (no framework — the right call now). But the backlog includes a **prescribed unified workflow screen**: a single surface with note editing, all CDI flags + scribe action items, status, and telemetry/edit-time tracking (for compliance/tracking — replacing today's "scribe uses the app/docs however they like, final edit lands in the EHR" free-for-all). That screen is stateful and data-dense — exactly where hand-rolled reactive DOM becomes the next monolith.
**Recommendation:** for *that* screen (not now), adopt a **mainstream** framework — **React** (industry default; best AI-coding support, so an agent builds screens in it most reliably and any web dev reads it) or **Vue** (most readable cold) — incrementally, one screen mounted via the existing `{mount,update,unmount}` seam, coexisting with the vanilla screens. **Explicitly avoid niche libraries (Preact, Lit, etc.):** their only advantage is small bundle size, which is irrelevant in desktop Electron, and they have weaker tooling/AI/community support. Overhead is ~zero until adopted; when adopted it's for one screen and the build step already exists (arrives with packaging/TypeScript, A2). The thing to avoid is hand-rolling a reactive state layer for the unified screen. This refactor's only obligation: keep the view seam framework-shaped so this is an addition, not a rewrite.
