...
AI Medical Notes/
  Cases/{patient_name}_{YYYY-MM-DD}/
    {patient_name}.mp3
    transcript.md
    {patient_name}_{YYYY-MM-DD}_soap_note.md   ← generated by Claude
  templates/                                    ← doctor templates (lowercase)
    sabbag.md
    harris.md
  .claude/                                      ← copied from notes-claude/ on first run
    skills/generate-note/SKILL.md
    settings.json
  app.log
```
...
Bundled in repo:
```
notes-claude/           ← copied to AI Medical Notes/.claude on first app startup
  skills/generate-note/SKILL.md
  settings.json
  scripts/
  draft/
...
spawn(PYTHON, [
  ...
  .on('close', code => {
    ...
    if (code === 0) spawnSoapGeneration(transcriptDest)
  })
```

**SOAP generation (step 10 — after transcription exits 0):**

```javascript
function spawnSoapGeneration(transcriptAbsPath) {
  // Path relative to NOTES_DIR — cwd when claude runs
  ...
  spawn('claude', ['-p', prompt, '--dangerously-skip-permissions'], {
    cwd: NOTES_DIR,
    stdio: ['ignore', 'pipe', 'pipe'],   // stdin must be 'ignore' — Claude waits if left open
    shell: process.platform === 'win32'  // needed for PATH resolution on Windows
  })
}
```

- `cwd: NOTES_DIR` — Claude must run from `AI Medical Notes/` for the skill in `.claude/skills/generate-note/` to be found
- `DOCTOR` is currently hardcoded (`'sabbag'`). Will be made configurable (per-session or per-case) in a future release.
- The skill (`SKILL.md`) reads the transcript, loads `templates/{doctor}.md`, generates a SOAP note following that doctor's exact format, and saves `{case}_soap_note.md` in the case folder.
- Fire-and-forget. All stdout/stderr logged to `app.log` under `[soap]` prefix.
- Requires `claude` CLI on PATH (`npm install -g @anthropic-ai/claude-code`)
- Requires `templates/{doctor}.md` to exist in `AI Medical Notes/templates/`
...
8. **Windows stop signal:** `recordingProcess.kill()` uses `TerminateProcess()` on Windows — Python gets no chance to flush WAV or run cleanup. Use stdin instead: write `'stop\n'` then `stdin.end()`. Python watches stdin in a background thread and sets `stop_event` cleanly.

9. **Claude stdin on Windows:** When spawning `claude` with `shell: true`, stdin is left open and Claude waits for input. Always set `stdio: ['ignore', 'pipe', 'pipe']`.

10. **`.claude` copy check:** Don't just check if `.claude/` dir exists — check for the skill file specifically (`SKILL.md`). A pre-existing empty `.claude` dir would otherwise skip the copy.

11. **Tray bounds on Linux:** `tray.getBounds()` may return `{x:0, y:0}`. Fall back to centering on `workArea`.
