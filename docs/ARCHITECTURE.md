...
**run-the-LLM has two modes.** Engines with no `runLlm` method (SOAP, CDI) run agentically via `ctx.llm.runSkill` (`claude -p`). Engines that expose `runLlm(input, ctx, caseCtx, {model, provider})` (ICD, em-score, patient-summary) instead run as a single Anthropic Messages-API call: `runEngine` branches on `!!engine.runLlm`, passes `ctx.api` (pinned Anthropic — never Gemini, model via `pinnedAnthropicModel()`), and the engine's `runLlm` reads inputs, calls `provider.runSingleCall`, writes its `_em.json`/`_patient_summary.json`, and returns a normalized `{code, text(=synthesized manifest), usage}` so the rest of the lifecycle is identical for both modes. See DECISIONS 2026-06-29. (These two need `ANTHROPIC_API_KEY` even on the "Agentic" SOAP option.)
...
User           Renderer            Main                Python              ElevenLabs       Claude CLI
 ...
 │               │                  │     runEngine(icd) → API: add-icd-codes-api candidates    │
 │               │                  │       → Node validates vs local FY2026 codeset (coder)    │
...
1. **`runEngine(icd, ctx, caseCtx)`** runs the ICD engine via its `runLlm` hook (like em-score/patient-summary), not agentically. Phase A: one Anthropic Messages-API call to the `add-icd-codes-api` skill (pinned to `ctx.api`) proposes billable candidate codes from the SOAP `.md`. Phase B: Node validates each candidate against the bundled FY2026 local codeset (`src/icd/coder.js` cross-check over `src/icd/lookup.js` → `data/icd/icd10cm_fy2026.db`, better-sqlite3, read-only) — accept only if it exists + is billable + description matches, else re-resolve via a codeset search, else flag for manual coding. Phase C: append (replace on a pre-chart re-run) the same `## ICD-10-CM Codes` table at the end of the file and write `<stem>_icd.json`. `runEngine` resolves on completion (success OR failure — it never rejects). A `processing_events` row with `job_kind='icd'` is recorded with token usage, cost, duration, and status (`success` / `failed` / `rate_limited`). Status-popup label transitions through `coding_icd` while it runs. **One gate short-circuits before the API call runs** — `icd.gates()` requires the global `enableIcd` setting on, else `runEngine` logs `[icd] SKIPPED: disabled`, reports `skipped`, and returns null with no call, no codes appended. The old agentic `add-icd-codes` skill + claude.ai ICD-10 MCP connector path is retired (see DECISIONS 2026-07-01).
...
- `add-icd-codes-api`: a single Anthropic Messages-API call (via the ICD engine's `runLlm`, not `buildPrompt`) that proposes candidate ICD-10-CM codes as JSON — no prompt-string signature. Node then validates the candidates against the bundled FY2026 local codeset (`src/icd/coder.js`). The agentic `add-icd-codes` skill (which emitted `ICD_OK`/`ICD_SKIPPED`/`ICD_ERROR` on stdout via the ICD-10 MCP connector) is retired (see DECISIONS 2026-07-01).
