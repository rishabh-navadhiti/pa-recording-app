...
| `cdi_flags` | `case_id`, `cdi_run_id`, `flag_index`, `type`, `category`, `title`, `body`, `guideline_reference`, `current_code`, `suggested_codes` (JSON), `confidence`, `evidence_found` (JSON), `evidence_missing` (JSON) | `db/cdi_flags.js` — bulk-inserted by `spawnCdiReview` on a successful run (manifest `status:'ok'` OR filesystem-fallback recovery). Attached to the case row that owns the SOAP it flagged (never to multi-patient parent rows). |
