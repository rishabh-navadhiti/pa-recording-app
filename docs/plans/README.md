...
| [2026-06-08-rs-cdi-manual-tab.md](2026-06-08-rs-cdi-manual-tab.md) | rs | Planned (new **CDI** tab — run `cdi-review` on a user-supplied SOAP note: paste/`.md`/`.docx` → ephemeral temp folder → CDI `.docx` to save anywhere. No storage; independent of global `enableCdi`/`cdiMode`. On `feature/cdi-manual-tab`. Depends on a user-supplied `python/docx_to_md.py`.) |
