...
async function generateSoapViaApi(transcriptAbsPath, soapNoteMdPath, caseTag, isRetry, templatePath, caseId, model, provider = null, providerName = 'API') {
  ...
  if (multiPatient) {
    ...
    if (!dResult.ok) {
      const keyEnvName = providerName === 'OpenAI' ? 'OPENAI_API_KEY' : providerName === 'Gemini' ? 'GEMINI_API_KEY' : 'ANTHROPIC_API_KEY'
    ...
    if (dManifest?.schema_version === 1 && dManifest.multi_patient && Array.isArray(dManifest.cases) && dManifest.cases.length >= 2) {
      ...
      await _runMultiPatientFanOut({
        ...
        model, provider, providerName, fail, prechartText,
      })
      return
    }
  ...
  const { system, user } = buildSingleCallNoteGen({
    ...
    doctorLastname, patientName: enteredName, dateOfService, prechartText,
  })
...
function registerIpcHandlers(appCtx) {
  ...
  const deps = {
    ...
    readEnv, writeEnvKey, validateElevenLabsKey, validateAnthropicKey, validateGeminiKey, validateOpenAiKey,
